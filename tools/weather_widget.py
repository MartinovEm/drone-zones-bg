...
window.wxCloudBuild = function(cb){
  ...
  ['msg_fes:clm', 'mtg_fd:ir105_hrfi&styles=mtg_fd_ir105_hrfi_grayscale'].forEach(function(q, i){
    ...
    im.src = base + q; imgs[i] = im; }); // on error nothing shows; the caller retries in 10 min
  function build(){
...
window.wxFcFetch = function(cb){
  ...
  function retry(){ // self-retry with doubling delay (12 s .. 5 min) — the grid is heavy, be polite
    clearTimeout(S.rt); S.rt = setTimeout(function(){ if (S.on) window.wxFcFetch(cb); }, S.rms || 12000);
...
function notRefresh(){
  if (!window.wxNotamOn || !map.getSource('wxnotam')) return;
  notFetch(function(gj, pts){
...
window.wxSetWind = function(on){ windOn = on; window.wxWindState.on = on;
  if (on){ wxBtnBusy('wxwind', '__never__'); // no map source (canvas) -> cleared in the fetch callback below / timeout
    ...
    window.wxWindFetch(function(){ if (!windOn) return; if (windEngine) windEngine.stop();
      windEngine = window.wxWindEngine({canvas: windCanvas, project: windProject, unproject: windUnproject, bounds: windBounds, darkBg: windDark, afterDraw: (typeof windMask !== 'undefined' ? windMask : null)}); wxBtnIdle('wxwind'); });
