...
window.wxSetMode = function(m){
  if (m === wxMode || !map.isStyleLoaded()) { if (m !== wxMode){ map.once('idle', function(){ window.wxSetMode(m); }); map.triggerRepaint(); } return; }
  wxMode = m; var d2 = (m === '2d');
  try{ ST.set('mode', m); }catch(e){} // remember the mode so a refresh restores it
  // The basemap (which tiles show) is owned by the ⧉ selector (wxApplyBasemap), NOT by
  // the mode. The mode only toggles the 3rd dimension: terrain + tilt + our 3D buildings.
  // So the chosen basemap (and its light/dark styling via .wx2d/wxDark) stays across 2D<->3D.
